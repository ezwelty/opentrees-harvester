...
class ScientificName {
  ...
  static fromFields(obj) {
    ...
    ScientificName._secondaryFields.forEach(key => {
      ...
      if (value) {
        if (key === 'genus') {
          value = helpers.toSentenceCase(helpers.latinize(value))
        } else if (key === 'species') {
          value = helpers.latinize(value).toLowerCase()
        } else if (key === 'cultivar') {
          value = helpers.toTitleCase(helpers.latinize(value))
        }
        if (key === 'species' && value.match(/^([x×]|hybrid)$/)) {
          names.hybrid = true
        } else if (!names[key]) {
          names[key] = value
        }
