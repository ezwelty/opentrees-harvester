...
// async function downloadWfsFeatureLayer({url, maxDays} = {}) {
//   // Check that we have not recently downloaded this URL
//   const existing = await archive.search({url}, {maxDays})
//   if (existing.length > 0) {
//     console.log(`[${url}] Already downloaded`)
...
//   // Get capabilities
//   const getCapabilitiesURL = wfs.buildGetCapabilitiesURL(url)
//   const response = await axios.get(getCapabilitiesURL, {transformResponse: [], validateStatus: () => true})
//   if (response.status != 200) {
//     console.error(`[${getCapabilitiesURL}] HTTP ${response.status}`)
//     return false
//   }
//   const capabilities = wfs.parseCapabilities(response.data)
...
//     if (!getFeatureURL) {
//       try {
//         getFeatureURL = wfs.buildGetFeatureURL(url, capabilities)
//       } catch (error) {
//         console.error(error.message)
//         return false
//       }
//     }
//     featureResponse = await axios.get(getFeatureURL, {transformResponse: [], validateStatus: () => true})
//     if (featureResponse.status != 200) {
//       console.error(`[${getFeatureURL}] HTTP ${response.status}`)
//       return false
//     }

//     // Write to file
//     tempfile = path.join(tempdir, `response-${counter}.gml`)
//     fs.writeFileSync(tempfile, featureResponse.data, {encoding: 'utf-8'})
//     tempfiles.push(tempfile)

//     result = wfs.parseFeatureResult(featureResponse.data)
//     if (result.next) {
//       getFeatureURL = result.next
//       console.log(`[${getFeatureURL}] Paging...`)
//       counter += 1
//     } else {
//       paging = false
//     }
...
//   // Merge files
//   tempfile = path.join(tempdir, 'response.gml')
//   wfs.mergeGMLFiles(tempfiles, tempfile)
//   for (mergedFile of tempfiles) {
//     fs.rmSync(mergedFile)
//   }
...
//     type: 'data',
//     api: 'wfs',
//     checksum: await archive.hashFile(tempfile)
//   }
//   const matching = await archive.search({ checksum: entry.checksum })
//   if (matching.length > 0) {
//     // Reuse path of most recent entry with matching checksum
...
//     const dir = archive.buildPath({url, date})
//     fs.mkdirSync(dir, { recursive: true })
//     entry.path = path.join(dir, path.basename(tempfile))
//     fs.renameSync(tempfile, entry.path)
//   }
