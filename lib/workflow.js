 ...
 * @returns {Promise<object>} Log entry
 */
async function downloadArcgisFeatureLayer({url, maxDays, type = 'data'} = {}) {
  // Check that we have not recently downloaded this URL
  if (maxDays !== 0) {
    const existing = await archive.search({url}, {maxDays})
    if (existing.length > 0) {
 ...
 * @param {number} params.maxDays - Maximum age of existing result in days that
 * would prevent downloading again
 * @param {string} [params.type='metadata'] - Type of data being downloaded
 * (e.g. 'data', 'metadata')
 * @returns {Promise<object>} Log entry
 */
async function downloadPage({url, format, maxDays, type = 'metadata'} = {}) {
  ...
  // Check that we have not recently downloaded this URL
  if (maxDays !== 0) {
    const existing = await archive.search({url, format}, {maxDays})
    if (existing.length > 0) {
      return existing[0]
    }
  }
  ...
  const entry = { url, date, type, format, checksum }
  const matching = await archive.search({ checksum: entry.checksum })
  if (matching.length > 0) {
    // Reuse path of most recent entry with matching checksum
    entry.path = matching[0].path
