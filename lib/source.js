...
class Source {
   ...
   * @return {Promise<string[]>} Resolves to the paths of the downloaded local
   * files (if any).
   */
    ...
    return await Promise.all(
      links.map(async link => {
        if (link.checksum) {
          ...
          const url = typeof link === 'string' ? link : Object.values(link)[0]
          const api = typeof link === 'string' ? null : Object.keys(link)[0]
          return await this.getUrl(url, {api, maxDays, type: 'data'})
        }
      })
    )
  }
   ...
   * files (if any).
   */
  async getMetadata({maxDays, format = 'mhtml'} = {}) {
    let links = this.props.info || []
    if (!Array.isArray(links)) {
      links = [links]
    }
    if (this.props.license?.url) {
      links.push(this.props.license.url)
    }
    if (!links.length) {
      return []
    }
    return await Promise.all(
      links.map(async link => {
        if (link.checksum) {
          ...
          const api = typeof link === 'string' ? null : Object.keys(link)[0]
          if (api) {
            return await this.getUrl(url, {api, maxDays, type: 'metadata'})
          }
          // Find or download webpage
          const entries = await archive.search({url, format}, {maxDays})
          if (entries.length) {
            this.success('URL found in archive:', entries[0].path)
            return entries[0].path
          }
          const entry = await workflow.downloadPage({url, format, maxDays: 0})
