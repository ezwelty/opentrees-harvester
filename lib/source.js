 ...
 * - { opendatasoft: 'url' } : OpenDataSoft API URL (treated as a plain URL)
 * - { socrata: 'url' } : Socrata API URL (treated as a plain URL)
  ...
  validate(error = false) {
    ...
    if (props.download) {
      ...
      if (!download.every(link => typeof link === 'string' || (
        ...
        ['checksum', 'manual', 'arcgis', 'wfs', 'opendatasoft', 'socrata'].includes(Object.keys(link)[0])
      ))) {
