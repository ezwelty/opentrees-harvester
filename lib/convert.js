...
function modifyCrosswalk(crosswalk) {
  ...
  for (const old in crosswalk) {
    ...
    target[old] = typeof crosswalk[old] === 'string' ?
      x => cleanFeatureField(x[crosswalk[old]]) :
      x => cleanFeatureField(crosswalk[old])
    if (conversions) {
      for (const name in conversions) {
        ...
        if (typeof conversions[name] === 'function') {
          ...
          target[name] = x => conversions[name](target[old](x))
          // NOTE: Always function with injection of cleanFeatureField()
          // if (typeof target[old] === 'function') {
          // } else {
          //   target[name] = x => conversions[name](x[target[old]])
          // }
        } else {
          // Rename field
          target[name] = target[old]
        }
      }
    }
  }
