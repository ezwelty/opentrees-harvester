...
function readCSV(path, options) {
  return new Promise((resolve, reject) => {
    ...
    fs.createReadStream(path).
      ...
      on('end', () => {
        console.log(`${path} successfully parsed`)
        resolve(rows)
      }).
      on('error', reject)
  })
 ...
 * @param {string} [path=null] – Path to file.
 * @returns {string|null} CSV as a string (if `path` is `null`).
 * @example
 * objects = [ { x: 1 }, { y: 2 }, { x: 3, y: 3, z: 'foo,bar' } ]
 * writeCSV(objects)
 * // 'x,y,z\n1,,\n,2,\n3,3,"foo,bar"'
 */
function writeCSV(objects, path = null) {
