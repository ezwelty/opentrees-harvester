...
function doesArchiverContainHash(hash) {
  const stream = fs.createReadStream("arhive.jsonl");
  const reader = readline.createInterface({input: stream});

  // All we're doing here is reading the archive.jsonl file line by line and turning it into a json object. And then we're checking if the .path members include the hash.
  // If it includes the hash, we don't need to download the data.
  reader.on('line', (line) => {
    const jsonObject = JSON.parse(line);
    if (jsonObject.path && jsonObject.path.includes(hash)) {
      return true;
    }
  })
  reader.on('close', () => {
    console.log("done reading archive.jsonl");
    return false;
  })
  return false;
