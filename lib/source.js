  ...
  async get(overwrite = false) {
    ...
    try {
      ...
      if (this.props.featureLayer) {
        const features = await helpers.getLayerFeatures(this.props.featureLayer)
        const output = path.join(this.dir, 'features.json')
        fs.writeFileSync(output, JSON.stringify(features))
