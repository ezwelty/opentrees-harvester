 ...
 * @property {string|string[]|object|object[]} data - Links to the
 ...
 * @property {string|string[]|object|object[]} metadata - Links to webpages or
 * files as one or more:
 * - string: URL of webpage to save as MHTML
 * - { checksum: 'checksum'} : Archive file checksum
 * - { file: 'url' } : URL to download (as with `data`)
...
class Source {
  ...
  async getData({maxDays, download = true} = {}) {
    const links = this.formatLinks(this.props.data, { type: 'data' })
    if (!links.length) {
      return []
    }
    return await Promise.all(
      links.map(async link => {
        ...
        file = this.retrieveFileFromArchive(
  ...
  async getMetadata({maxDays, format = 'mhtml', download = true} = {}) {
    const links = this.formatLinks(this.props.metadata, { type: 'metadata' })
   ...
   * @private
   */
  retrieveFileFromArchive({ url, api, type, format, maxDays } = {}) {
    let entries
    if (api === 'checksum') {
      entries = archive.search({ checksum: url })
    } else {
      const filters = {
        url, ...type && {type}, ...format && api === 'browser' && {format}
      }
      entries = archive.search(filters, { maxDays })
    }
  ...
  async sample(options = {}) {
    ...
    const types = await this.getFields()
    const values = {}
  ...
  async glimpse(options = {}) {
    ...
    if (!options.sample) {
      options.sample = await this.sample(options)
    }
    const tableOptions = {
      columnDefault: {
        wrapWord: true,
