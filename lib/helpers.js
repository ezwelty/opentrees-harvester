...
/**
 * This function searches through the archiver line by line and checks if any of the entries contain the data_hash already.
 * If so, then return true and the entry that contains the hash. If not, then we will return false with an empty line.
 * @param {string} hash the data hash using md5
 * @returns array with array[0] being true/false and array[1] equaling an entry in the archiver (or an empty string is array[0] === false)
 */

async function doesArchiverContainHash(hash) {
  const stream = fs.createReadStream("archive.jsonl");
  const reader = readline.createInterface({input: stream});

  let hashEqual = false;
  let lineThatContainsHash = "";

  // All we're doing here is reading the archive.jsonl file line by line and turning it into a json object. And then we're checking if the data at the jsonObject.data_hash is equal to the hash.
  for await (const line of reader) {
    const jsonObject = JSON.parse(line);
    if (jsonObject.data_hash && jsonObject.data_hash === hash) {
      hashEqual = true;
      lineThatContainsHash = jsonObject;
    }
  }
  return [hashEqual, lineThatContainsHash];
