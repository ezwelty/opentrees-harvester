...
function makeCrosswalkMapping(headers, crosswalk_dataset) {
    ...
    for (const header of headers) {
        ...
        if (!Object.keys(crosswalk).includes(similar_headers.toString()) && !Object.keys(crosswalk).includes(matched_headers.toString())) {
            ...
            if (typeof matched_headers == 'string') {
                if (typeof similar_headers != 'string') {
                    ignored_bc_existing[similar_headers] = header
                }
            } else {
                if (typeof similar_headers != 'string') {
                    ignored_bc_existing[all_headers] = header
                } else {
                    ignored_bc_existing[matched_headers] = header
                }
            }
        }
    }
    ...
    for (const ignored of Object.entries(ignored_bc_existing)) {
        console.log('// The guesser matched "'+ ignored[1] + '" to the the possible target header(s) [' + ignored[0] + ']. But [' + ignored[0] + '] was previously matched to another entry.')
    }
...
console.log(makeCrosswalkMapping(['dbhffffcecomment','f','species','reff','ref','treetypeifhosdf'],sorted_data))
