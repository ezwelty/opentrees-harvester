...
class Matcher {
    ...
    if (name.species) {
      if (name.species in node) {
        ...
        if (fuzzy) {
          ...
          return { incomplete: true, ...fuzzy && { fuzzy: scores }, taxon: node[''] }
        }
      }
    } else {
      return { ...fuzzy && { fuzzy: scores }, taxon: node[''] }
    }
    // infraspecies
  ...
  match(name) {
    const matches = []
    const normalized = this._matchNormalized(name)
    if (normalized) {
      matches.push(normalized)
    }
    if (!normalized || normalized.fuzzy || normalized.incomplete) {
