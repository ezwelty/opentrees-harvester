 ...
 * @property {string|string[]|ArchiveFile|ArchiveFile[]} download - Links to the
 * data file(s) as either a single URL, multiple URLs, or an archive file checksum.
 * @property {string} api - API endpoint type. If `arcgis`, `download` URL is
 * treated as an ArcGIS Feature Server layer. If `manual`, `download` is ignored
 * (e.g. download is left to the user). In all other cases, `download` is
 * treated as a standard file download.
 ...
 * @property {string|string[]|ArchiveFile} info - Links to webpages or files
 * as either a single URL, multiple URLs, or an archive file checksum.
  ...
  validate(error = false) {
    ...
    if (props.download) {
      if (!(
        ...
        (Array.isArray(props.download) && props.download.every(x => typeof x === 'string')) ||
        (typeof props.download === 'object' && props.download.checksum)
      )) {
        errors.push(['Invalid download', props.download])
      }
    }
    // api
    if (props.api && typeof props.api !== 'string') {
      errors.push(['Invalid api', props.api])
    }
    // vfs
    if (props.vfs) {
      if (
    ...
    if (props.info) {
      if (!(
        ...
        (Array.isArray(props.info) && props.info.every(x => typeof x === 'string')) ||
        (typeof props.info === 'object' && props.info.checksum)
      )) {
