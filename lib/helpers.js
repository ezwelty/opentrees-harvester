...
/**
 * Convert GDAL list to string.
 *
 * @param {Array} list - Array
 * @returns {string} List as a pipe-delimited string
 */
...
const gdalStringFormatters = {
  ...
  [gdal.OFTRealList]: listToString,
  [gdal.OFTIntegerList]: listToString,
  [gdal.OFTInteger64List]: listToString,
  [gdal.OFTStringList]: listToString,
  [gdal.OFTWideStringList]: listToString
}
...
function getTransform(source, target) {
  ...
  const sameAxes = isAxesXY(source) === isAxesXY(target)
  if (!(same && sameAxes)) {
...
function isAxesXY(srs) {
  ...
  wkt = srs.toWKT()
  xi = wkt.match(/axis\[[^\]]*(east|west|longitude)[^\]]*\]/i)
  yi = wkt.match(/axis\[[^\]]*(north|south|latitude)[^\]]*\]/i)
  if (xi && xi) {
...
/**
 * Determine whether geometry x, y has swapped axes relative to a target.
 *
 * @param {number[]} xy - Coordinates x, y.
 * @param {number[]} targetXY - Target coordinates x, y.
 * @returns {boolean} Whether x, y has swapped axes relative to target x, y.
 */
function hasSwappedAxes(xy, targetXY) {
  return (
    (Math.abs(xy[0] - targetXY[0]) > Math.abs(xy[0] - targetXY[1])) ||
    (Math.abs(xy[1] - targetXY[1]) > Math.abs(xy[1] - targetXY[0]))
  )
}
...
function writeCSV(objects, file = null) {
  ...
  const rows = objects.map(obj => {
    return columns.map(key => {
      const x = obj[key]
      const type = typeof x
      if (x == null || (type === 'number' && isNaN(x))) {
        return ''
      }
      return (type === 'string' && x.includes(',')) ? `"${x}"` : String(x)
    }).join(',')
  })
  // Combine
 ...
 * @param {object} [options]
 * @param {string} [options.type] – Geometry type (e.g. 'wkbPoint').
 * @param {string} [options.srs] – Spatial reference system (e.g. '+init=EPSG:4326').
 * @param {object} [options.geometry] – Geometry mapping
 * (e.g. { x: 'lon', y: 'lat' } or { wkt: 'geom' }).
 * @returns {gdal.Dataset}
 */
function openFileUnionWithGdal(files, { type, srs, geometry } = {}) {
  ...
  for (const file of files) {
    ...
    if (type) {
      vrt += `<GeometryType>${type}</GeometryType>`
    }
    if (srs) {
      vrt += `<LayerSRS>${srs}</LayerSRS>`
    }
    if (geometry) {
      if (geometry.wkt) {
        vrt += `<GeometryField encoding="WKT" field="${geometry.wkt}"/>`
      } else if (geometry.x && geometry.y) {
        vrt += `<GeometryField encoding="PointFromColumns" x="${geometry.x}" y="${geometry.y}"/>`
      } else {
        throw new Error(`Unsupported geometry mapping:`, geometry)
      }
    }
    vrt += `</OGRVRTLayer>`
  }
